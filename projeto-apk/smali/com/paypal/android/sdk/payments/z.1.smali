.class final Lcom/paypal/android/sdk/payments/z;
.super Ljava/lang/Object;


# instance fields
.field a:Landroid/view/View;

.field b:Landroid/widget/TextView;

.field c:Landroid/widget/Button;

.field private d:Z

.field private e:Landroid/widget/ScrollView;

.field private f:Landroid/widget/LinearLayout;

.field private g:Landroid/widget/ImageView;


# direct methods
.method constructor <init>(Landroid/content/Context;Z)V
    .locals 13

    const/4 v12, 0x0

    const/4 v11, -0x2

    const/4 v10, -0x1

    const/4 v9, 0x1

    const/4 v8, 0x0

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    iput-boolean p2, p0, Lcom/paypal/android/sdk/payments/z;->d:Z

    new-instance v0, Landroid/widget/ScrollView;

    invoke-direct {v0, p1}, Landroid/widget/ScrollView;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/paypal/android/sdk/payments/z;->e:Landroid/widget/ScrollView;

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->e:Landroid/widget/ScrollView;

    sget v1, Lcom/paypal/android/sdk/bk;->c:I

    invoke-virtual {v0, v1}, Landroid/widget/ScrollView;->setBackgroundColor(I)V

    new-instance v0, Landroid/widget/LinearLayout;

    invoke-direct {v0, p1}, Landroid/widget/LinearLayout;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/paypal/android/sdk/payments/z;->f:Landroid/widget/LinearLayout;

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->f:Landroid/widget/LinearLayout;

    invoke-virtual {v0, v9}, Landroid/widget/LinearLayout;->setOrientation(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->e:Landroid/widget/ScrollView;

    iget-object v1, p0, Lcom/paypal/android/sdk/payments/z;->f:Landroid/widget/LinearLayout;

    invoke-virtual {v0, v1}, Landroid/widget/ScrollView;->addView(Landroid/view/View;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->f:Landroid/widget/LinearLayout;

    invoke-static {v0, v10, v11}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;II)V

    new-instance v2, Landroid/widget/LinearLayout;

    invoke-direct {v2, p1}, Landroid/widget/LinearLayout;-><init>(Landroid/content/Context;)V

    invoke-virtual {v2, v9}, Landroid/widget/LinearLayout;->setOrientation(I)V

    const-string v0, "16dip"

    const-string v1, "20dip"

    const-string v3, "16dip"

    const-string v4, "20dip"

    invoke-static {v2, v0, v1, v3, v4}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->f:Landroid/widget/LinearLayout;

    invoke-virtual {v0, v2}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    invoke-static {v2, v10, v11}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;II)V

    new-instance v0, Landroid/widget/ImageView;

    invoke-direct {v0, p1}, Landroid/widget/ImageView;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/paypal/android/sdk/payments/z;->g:Landroid/widget/ImageView;

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->g:Landroid/widget/ImageView;

    const-string v1, ""

    invoke-static {v1, p1}, Lcom/paypal/android/sdk/bl;->c(Ljava/lang/String;Landroid/content/Context;)Landroid/graphics/Bitmap;

    move-result-object v1

    invoke-virtual {v0, v1}, Landroid/widget/ImageView;->setImageBitmap(Landroid/graphics/Bitmap;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->g:Landroid/widget/ImageView;

    const-string v1, "PayPal"

    invoke-virtual {v0, v1}, Landroid/widget/ImageView;->setContentDescription(Ljava/lang/CharSequence;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->g:Landroid/widget/ImageView;

    invoke-virtual {v2, v0}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->g:Landroid/widget/ImageView;

    const-string v1, "40dip"

    invoke-static {v0, v10, v1}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;ILjava/lang/String;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->g:Landroid/widget/ImageView;

    const-string v1, "10dip"

    invoke-static {v0, v8, v8, v8, v1}, Lcom/paypal/android/sdk/bl;->b(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    new-instance v0, Landroid/widget/TextView;

    invoke-direct {v0, p1}, Landroid/widget/TextView;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/paypal/android/sdk/payments/z;->b:Landroid/widget/TextView;

    new-instance v3, Landroid/webkit/WebView;

    invoke-direct {v3, p1}, Landroid/webkit/WebView;-><init>(Landroid/content/Context;)V

    sget v0, Lcom/paypal/android/sdk/bk;->c:I

    invoke-virtual {v3, v0}, Landroid/webkit/WebView;->setBackgroundColor(I)V

    sget-object v0, Lcom/paypal/android/sdk/bG;->x:Lcom/paypal/android/sdk/bG;

    invoke-static {v0}, Lcom/paypal/android/sdk/bF;->a(Lcom/paypal/android/sdk/bG;)Ljava/lang/String;

    move-result-object v4

    invoke-static {}, Ljava/util/Locale;->getDefault()Ljava/util/Locale;

    move-result-object v0

    invoke-virtual {v0}, Ljava/util/Locale;->getCountry()Ljava/lang/String;

    move-result-object v0

    sget-object v1, Ljava/util/Locale;->US:Ljava/util/Locale;

    invoke-virtual {v0, v1}, Ljava/lang/String;->toLowerCase(Ljava/util/Locale;)Ljava/lang/String;

    move-result-object v0

    invoke-static {v0}, Lcom/paypal/android/sdk/R;->c(Ljava/lang/CharSequence;)Z

    move-result v1

    if-eqz v1, :cond_0

    const-string v0, "us"

    :cond_0
    new-instance v5, Ljava/util/HashSet;

    const/4 v1, 0x6

    new-array v1, v1, [Ljava/lang/String;

    const-string v6, "nl"

    aput-object v6, v1, v12

    const-string v6, "de"

    aput-object v6, v1, v9

    const/4 v6, 0x2

    const-string v7, "it"

    aput-object v7, v1, v6

    const/4 v6, 0x3

    const-string v7, "fr"

    aput-object v7, v1, v6

    const/4 v6, 0x4

    const-string v7, "gb"

    aput-object v7, v1, v6

    const/4 v6, 0x5

    const-string v7, "us"

    aput-object v7, v1, v6

    invoke-static {v1}, Ljava/util/Arrays;->asList([Ljava/lang/Object;)Ljava/util/List;

    move-result-object v1

    invoke-direct {v5, v1}, Ljava/util/HashSet;-><init>(Ljava/util/Collection;)V

    const-string v1, ""

    invoke-interface {v5, v0}, Ljava/util/Set;->contains(Ljava/lang/Object;)Z

    move-result v5

    if-eqz v5, :cond_2

    const-string v1, "https://www.paypal.com/%s/webapps/mpp/ua/useragreement-full"

    new-array v5, v9, [Ljava/lang/Object;

    aput-object v0, v5, v12

    invoke-static {v1, v5}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v0

    :goto_0
    new-array v1, v9, [Ljava/lang/Object;

    aput-object v0, v1, v12

    invoke-static {v4, v1}, Ljava/lang/String;->format(Ljava/lang/String;[Ljava/lang/Object;)Ljava/lang/String;

    move-result-object v1

    iget-boolean v0, p0, Lcom/paypal/android/sdk/payments/z;->d:Z

    if-eqz v0, :cond_1

    const-string v0, "direction: rtl;"

    :goto_1
    new-instance v4, Ljava/lang/StringBuilder;

    const-string v5, "body { color: #515151; line-height: 26px; font-size: 18px; font-family: sans-serif-light, DroidSans, sans-serif; font-weight: 300; "

    invoke-direct {v4, v5}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v4, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    const-string v4, "} h1, strong { font-family: DroidSans, sans-serif; font-weight: 500; }"

    invoke-virtual {v0, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    const-string v4, "h1 { font-size: 18px }h1, p { margin: 0 0 13px 0 }"

    invoke-virtual {v0, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    const-string v4, "ul { margin: 0; padding-left: 1.1em; list-style-position: outside; }"

    invoke-virtual {v0, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    new-instance v4, Ljava/lang/StringBuilder;

    const-string v5, "<html><meta name=\'viewport\' content=\'width=device-width; initial-scale=1.0;\'/><head><style type=\"text/css\">"

    invoke-direct {v4, v5}, Ljava/lang/StringBuilder;-><init>(Ljava/lang/String;)V

    invoke-virtual {v4, v0}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    const-string v4, "</style></head><body>"

    invoke-virtual {v0, v4}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    const-string v1, "</body><html>"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    move-result-object v0

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    const-string v1, "text/html; charset=UTF-8"

    invoke-virtual {v3, v0, v1, v8}, Landroid/webkit/WebView;->loadData(Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    invoke-virtual {v3, v9}, Landroid/webkit/WebView;->setClickable(Z)V

    invoke-virtual {v2, v3}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    invoke-static {v3, v10, v11}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;II)V

    const-string v0, "10dip"

    invoke-static {v3, v8, v8, v8, v0}, Lcom/paypal/android/sdk/bl;->b(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    new-instance v0, Landroid/widget/Button;

    invoke-direct {v0, p1}, Landroid/widget/Button;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/paypal/android/sdk/payments/z;->c:Landroid/widget/Button;

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->c:Landroid/widget/Button;

    invoke-virtual {v2, v0}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->c:Landroid/widget/Button;

    invoke-static {v0, v9, p1}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;ZLandroid/content/Context;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->c:Landroid/widget/Button;

    const-string v1, "20dip"

    const-string v2, "4dip"

    invoke-static {v0, v8, v1, v8, v2}, Lcom/paypal/android/sdk/bl;->b(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/payments/z;->e:Landroid/widget/ScrollView;

    iput-object v0, p0, Lcom/paypal/android/sdk/payments/z;->a:Landroid/view/View;

    return-void

    :cond_1
    const-string v0, ""

    goto :goto_1

    :cond_2
    move-object v0, v1

    goto/16 :goto_0
.end method
