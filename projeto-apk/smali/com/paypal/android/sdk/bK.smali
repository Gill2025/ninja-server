.class public final Lcom/paypal/android/sdk/bK;
.super Ljava/lang/Object;


# instance fields
.field public a:Landroid/view/ViewGroup;

.field public b:Landroid/widget/EditText;

.field public c:Lcom/paypal/android/sdk/bJ;

.field public d:Landroid/widget/EditText;

.field public e:Landroid/widget/Button;

.field public f:Landroid/widget/LinearLayout;

.field public g:Landroid/widget/TextView;

.field public h:Landroid/widget/Button;

.field private i:Landroid/view/ViewGroup;

.field private j:Landroid/widget/LinearLayout;

.field private k:Landroid/widget/ImageView;


# direct methods
.method public constructor <init>(Landroid/content/Context;)V
    .locals 10

    const v9, 0xa80d

    const/4 v8, -0x1

    const/4 v5, 0x0

    const/4 v7, 0x1

    const/4 v6, -0x2

    invoke-direct {p0}, Ljava/lang/Object;-><init>()V

    new-instance v0, Landroid/widget/ScrollView;

    invoke-direct {v0, p1}, Landroid/widget/ScrollView;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/paypal/android/sdk/bK;->i:Landroid/view/ViewGroup;

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->i:Landroid/view/ViewGroup;

    sget v1, Lcom/paypal/android/sdk/bk;->c:I

    invoke-virtual {v0, v1}, Landroid/view/ViewGroup;->setBackgroundColor(I)V

    new-instance v0, Landroid/widget/LinearLayout;

    invoke-direct {v0, p1}, Landroid/widget/LinearLayout;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/paypal/android/sdk/bK;->j:Landroid/widget/LinearLayout;

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->j:Landroid/widget/LinearLayout;

    invoke-virtual {v0, v7}, Landroid/widget/LinearLayout;->setOrientation(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->i:Landroid/view/ViewGroup;

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->j:Landroid/widget/LinearLayout;

    invoke-virtual {v0, v1}, Landroid/view/ViewGroup;->addView(Landroid/view/View;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->j:Landroid/widget/LinearLayout;

    invoke-static {v0, v8, v6}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;II)V

    new-instance v0, Landroid/widget/LinearLayout;

    invoke-direct {v0, p1}, Landroid/widget/LinearLayout;-><init>(Landroid/content/Context;)V

    invoke-virtual {v0, v7}, Landroid/widget/LinearLayout;->setOrientation(I)V

    const-string v1, "16dip"

    const-string v2, "20dip"

    const-string v3, "16dip"

    const-string v4, "20dip"

    invoke-static {v0, v1, v2, v3, v4}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->j:Landroid/widget/LinearLayout;

    invoke-virtual {v1, v0}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    invoke-static {v0, v8, v6}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;II)V

    new-instance v1, Landroid/widget/ImageView;

    invoke-direct {v1, p1}, Landroid/widget/ImageView;-><init>(Landroid/content/Context;)V

    iput-object v1, p0, Lcom/paypal/android/sdk/bK;->k:Landroid/widget/ImageView;

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->k:Landroid/widget/ImageView;

    const-string v2, ""

    invoke-static {v2, p1}, Lcom/paypal/android/sdk/bl;->c(Ljava/lang/String;Landroid/content/Context;)Landroid/graphics/Bitmap;

    move-result-object v2

    invoke-virtual {v1, v2}, Landroid/widget/ImageView;->setImageBitmap(Landroid/graphics/Bitmap;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->k:Landroid/widget/ImageView;

    const-string v2, "PayPal"

    invoke-virtual {v1, v2}, Landroid/widget/ImageView;->setContentDescription(Ljava/lang/CharSequence;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->k:Landroid/widget/ImageView;

    invoke-virtual {v0, v1}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->k:Landroid/widget/ImageView;

    const-string v2, "40dip"

    invoke-static {v1, v8, v2}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;ILjava/lang/String;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->k:Landroid/widget/ImageView;

    const-string v2, "10dip"

    invoke-static {v1, v5, v5, v5, v2}, Lcom/paypal/android/sdk/bl;->b(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    new-instance v1, Landroid/widget/EditText;

    invoke-direct {v1, p1}, Landroid/widget/EditText;-><init>(Landroid/content/Context;)V

    iput-object v1, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    const v2, 0xa80c

    invoke-virtual {v1, v2}, Landroid/widget/EditText;->setId(I)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    invoke-virtual {v1, v7}, Landroid/widget/EditText;->setInputType(I)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    sget v2, Lcom/paypal/android/sdk/bk;->i:I

    invoke-virtual {v1, v2}, Landroid/widget/EditText;->setTextColor(I)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    sget-object v2, Lcom/paypal/android/sdk/bk;->o:Landroid/graphics/Typeface;

    invoke-virtual {v1, v2}, Landroid/widget/EditText;->setTypeface(Landroid/graphics/Typeface;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    invoke-virtual {v1, v7}, Landroid/widget/EditText;->setSingleLine(Z)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    invoke-virtual {v0, v1}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    invoke-static {v1, v8, v6}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;II)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    const-string v2, "10dip"

    invoke-static {v1, v5, v5, v5, v2}, Lcom/paypal/android/sdk/bl;->b(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    new-instance v1, Landroid/widget/EditText;

    invoke-direct {v1, p1}, Landroid/widget/EditText;-><init>(Landroid/content/Context;)V

    iput-object v1, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    invoke-virtual {v1, v9}, Landroid/widget/EditText;->setId(I)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    const/16 v2, 0x80

    invoke-virtual {v1, v2}, Landroid/widget/EditText;->setInputType(I)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    sget v2, Lcom/paypal/android/sdk/bk;->i:I

    invoke-virtual {v1, v2}, Landroid/widget/EditText;->setTextColor(I)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    sget-object v2, Lcom/paypal/android/sdk/bk;->o:Landroid/graphics/Typeface;

    invoke-virtual {v1, v2}, Landroid/widget/EditText;->setTypeface(Landroid/graphics/Typeface;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    invoke-virtual {v1, v7}, Landroid/widget/EditText;->setSingleLine(Z)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    invoke-virtual {v0, v1}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    invoke-static {v1, v8, v6}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;II)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    const-string v2, "10dip"

    invoke-static {v1, v5, v5, v5, v2}, Lcom/paypal/android/sdk/bl;->b(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    const v1, 0xa80e

    invoke-static {p1, v7, v1, v0}, Lcom/paypal/android/sdk/bl;->a(Landroid/content/Context;ZILandroid/widget/LinearLayout;)Landroid/widget/LinearLayout;

    move-result-object v1

    iput-object v1, p0, Lcom/paypal/android/sdk/bK;->f:Landroid/widget/LinearLayout;

    new-instance v1, Landroid/widget/TextView;

    invoke-direct {v1, p1}, Landroid/widget/TextView;-><init>(Landroid/content/Context;)V

    iput-object v1, p0, Lcom/paypal/android/sdk/bK;->g:Landroid/widget/TextView;

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->g:Landroid/widget/TextView;

    invoke-static {v1}, Lcom/paypal/android/sdk/bl;->a(Landroid/widget/TextView;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->g:Landroid/widget/TextView;

    const-string v2, "init"

    invoke-virtual {v1, v2}, Landroid/widget/TextView;->setText(Ljava/lang/CharSequence;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->f:Landroid/widget/LinearLayout;

    iget-object v2, p0, Lcom/paypal/android/sdk/bK;->g:Landroid/widget/TextView;

    invoke-virtual {v1, v2}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    new-instance v1, Landroid/widget/TableLayout;

    invoke-direct {v1, p1}, Landroid/widget/TableLayout;-><init>(Landroid/content/Context;)V

    invoke-virtual {v1, v7, v7}, Landroid/widget/TableLayout;->setColumnStretchable(IZ)V

    const/4 v2, 0x0

    invoke-virtual {v1, v2, v7}, Landroid/widget/TableLayout;->setColumnShrinkable(IZ)V

    invoke-virtual {v0, v1}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    invoke-static {v1, v8, v6}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;II)V

    new-instance v0, Landroid/widget/TableRow;

    invoke-direct {v0, p1}, Landroid/widget/TableRow;-><init>(Landroid/content/Context;)V

    invoke-virtual {v1, v0}, Landroid/widget/TableLayout;->addView(Landroid/view/View;)V

    invoke-static {v0, v8, v6}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;II)V

    new-instance v1, Landroid/widget/Button;

    invoke-direct {v1, p1}, Landroid/widget/Button;-><init>(Landroid/content/Context;)V

    iput-object v1, p0, Lcom/paypal/android/sdk/bK;->e:Landroid/widget/Button;

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->e:Landroid/widget/Button;

    const v2, 0xa80f

    invoke-virtual {v1, v2}, Landroid/widget/Button;->setId(I)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->e:Landroid/widget/Button;

    invoke-virtual {v0, v1}, Landroid/widget/TableRow;->addView(Landroid/view/View;)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->e:Landroid/widget/Button;

    const/4 v2, 0x3

    invoke-static {v1, v2}, Lcom/paypal/android/sdk/bl;->a(Landroid/widget/Button;I)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->e:Landroid/widget/Button;

    const-string v2, "0dp"

    invoke-static {v1, v2, v6}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;Ljava/lang/String;I)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->e:Landroid/widget/Button;

    const/4 v2, 0x3

    const/high16 v3, 0x3f000000    # 0.5f

    invoke-static {v1, v2, v3}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;IF)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->e:Landroid/widget/Button;

    const-string v2, "4dip"

    const-string v3, "2dip"

    const-string v4, "2dip"

    const-string v5, "2dip"

    invoke-static {v1, v2, v3, v4, v5}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    new-instance v1, Landroid/widget/Button;

    invoke-direct {v1, p1}, Landroid/widget/Button;-><init>(Landroid/content/Context;)V

    iput-object v1, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    const v2, 0xa810

    invoke-virtual {v1, v2}, Landroid/widget/Button;->setId(I)V

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    invoke-virtual {v0, v1}, Landroid/widget/TableRow;->addView(Landroid/view/View;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    const/4 v1, 0x5

    invoke-static {v0, v1}, Lcom/paypal/android/sdk/bl;->a(Landroid/widget/Button;I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    const-string v1, "0dp"

    invoke-static {v0, v1, v6}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;Ljava/lang/String;I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    const/4 v1, 0x5

    const/high16 v2, 0x3f000000    # 0.5f

    invoke-static {v0, v1, v2}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;IF)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    const-string v1, "2dip"

    const-string v2, "2dip"

    const-string v3, "4dip"

    const-string v4, "2dip"

    invoke-static {v0, v1, v2, v3, v4}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;Ljava/lang/String;)V

    invoke-static {}, Ljava/util/Locale;->getDefault()Ljava/util/Locale;

    move-result-object v0

    invoke-virtual {v0}, Ljava/util/Locale;->getCountry()Ljava/lang/String;

    move-result-object v0

    sget-object v1, Ljava/util/Locale;->US:Ljava/util/Locale;

    invoke-virtual {v0, v1}, Ljava/lang/String;->toLowerCase(Ljava/util/Locale;)Ljava/lang/String;

    move-result-object v0

    invoke-static {v0}, Lcom/paypal/android/sdk/s;->f(Ljava/lang/String;)Z

    move-result v0

    if-nez v0, :cond_0

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    const/4 v1, 0x4

    invoke-virtual {v0, v1}, Landroid/widget/Button;->setVisibility(I)V

    :cond_0
    new-instance v0, Lcom/paypal/android/sdk/bJ;

    invoke-direct {v0, p1}, Lcom/paypal/android/sdk/bJ;-><init>(Landroid/content/Context;)V

    iput-object v0, p0, Lcom/paypal/android/sdk/bK;->c:Lcom/paypal/android/sdk/bJ;

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->j:Landroid/widget/LinearLayout;

    iget-object v1, p0, Lcom/paypal/android/sdk/bK;->c:Lcom/paypal/android/sdk/bJ;

    iget-object v1, v1, Lcom/paypal/android/sdk/bJ;->a:Landroid/widget/LinearLayout;

    invoke-virtual {v0, v1}, Landroid/widget/LinearLayout;->addView(Landroid/view/View;)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->c:Lcom/paypal/android/sdk/bJ;

    iget-object v0, v0, Lcom/paypal/android/sdk/bJ;->a:Landroid/widget/LinearLayout;

    invoke-static {v0, v6, v6}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;II)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->c:Lcom/paypal/android/sdk/bJ;

    iget-object v0, v0, Lcom/paypal/android/sdk/bJ;->a:Landroid/widget/LinearLayout;

    const/16 v1, 0x11

    const/high16 v2, 0x3f800000    # 1.0f

    invoke-static {v0, v1, v2}, Lcom/paypal/android/sdk/bl;->a(Landroid/view/View;IF)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    invoke-virtual {v0, v9}, Landroid/widget/EditText;->setNextFocusDownId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    const v1, 0xa80c

    invoke-virtual {v0, v1}, Landroid/widget/EditText;->setNextFocusLeftId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->b:Landroid/widget/EditText;

    const v1, 0xa80c

    invoke-virtual {v0, v1}, Landroid/widget/EditText;->setNextFocusRightId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    const v1, 0xa80c

    invoke-virtual {v0, v1}, Landroid/widget/EditText;->setNextFocusUpId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    const v1, 0xa80e

    invoke-virtual {v0, v1}, Landroid/widget/EditText;->setNextFocusDownId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    invoke-virtual {v0, v9}, Landroid/widget/EditText;->setNextFocusLeftId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->d:Landroid/widget/EditText;

    invoke-virtual {v0, v9}, Landroid/widget/EditText;->setNextFocusRightId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->f:Landroid/widget/LinearLayout;

    invoke-virtual {v0, v9}, Landroid/widget/LinearLayout;->setNextFocusUpId(I)V

    invoke-virtual {p0, v7}, Lcom/paypal/android/sdk/bK;->a(Z)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->e:Landroid/widget/Button;

    const v1, 0xa80e

    invoke-virtual {v0, v1}, Landroid/widget/Button;->setNextFocusUpId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->e:Landroid/widget/Button;

    const v1, 0xa810

    invoke-virtual {v0, v1}, Landroid/widget/Button;->setNextFocusRightId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    const v1, 0xa80e

    invoke-virtual {v0, v1}, Landroid/widget/Button;->setNextFocusUpId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->i:Landroid/view/ViewGroup;

    iput-object v0, p0, Lcom/paypal/android/sdk/bK;->a:Landroid/view/ViewGroup;

    return-void
.end method


# virtual methods
.method public final a(Z)V
    .locals 2

    const v1, 0xa80f

    if-eqz p1, :cond_0

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->f:Landroid/widget/LinearLayout;

    invoke-virtual {v0, v1}, Landroid/widget/LinearLayout;->setNextFocusDownId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    invoke-virtual {v0, v1}, Landroid/widget/Button;->setNextFocusLeftId(I)V

    :goto_0
    return-void

    :cond_0
    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->f:Landroid/widget/LinearLayout;

    const v1, 0xa810

    invoke-virtual {v0, v1}, Landroid/widget/LinearLayout;->setNextFocusDownId(I)V

    iget-object v0, p0, Lcom/paypal/android/sdk/bK;->h:Landroid/widget/Button;

    const v1, 0xa80e

    invoke-virtual {v0, v1}, Landroid/widget/Button;->setNextFocusLeftId(I)V

    goto :goto_0
.end method
